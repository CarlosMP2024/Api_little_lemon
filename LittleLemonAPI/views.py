...
from .serializers import CategorySerializer,MenuItemSerializer,CartSerialzer,OrderSerializer,OrderItemSerializer,ManagerListSerializer,CartAddSerializer,CartRemoveSerializer,OrderInsertSerializer,SingleOrderSerializer
...
class DeliveryCrewView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if username:
            user = get_object_or_404(User, username=username)
            crew = Group.objects.get(name='DeliveryCrew')
...
class CartView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        existing_cart_item = Cart.objects.filter(user=request.user, menuitem_id=menuitem_id).first()
        ...
        try:
            ...
            return Response({'message': 'Invalid menuitem id'}, status=status.HTTP_404_NOT_FOUND)

        price = int(quantity) * item.price
        Cart.objects.create(user=request.user, quantity=quantity, unit_price=item.price, price=price, menuitem_id=menuitem_id)
...
class SingleOrderView(generics.RetrieveUpdateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        query = OrderItem.objects.filter(order_id=self.kwargs['pk'])
        serializer = SingleOrderSerializer(query, many=True)
